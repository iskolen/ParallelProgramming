...
void WriteDataToFile(FILE* outputFile, int threadId, DWORD elapsedTime)
{
...
void ApplyGaussianBlur(void* threadData)
{
	...
	std::string fileName = "outputThread" + std::to_string(threadId) + ".csv";
	FILE* outputFile;
	if (fopen_s(&outputFile, fileName.c_str(), "w") != 0)
	{
	...
	for (int y = 0; y < imageHeight; ++y)
	{
		for (int x = startColumn; x < endColumn; ++x)
		{
			...
			DWORD elapsedTime = endTime - data->startTime;
			WriteDataToFile(outputFile, threadId, elapsedTime);
		}
	}
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < numThreads; ++i)
	{
		...
		handles[i] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ApplyGaussianBlur,
			new ThreadData{ std::ref(imageData), start, end, header.width, header.height, i, startTime }, 0, NULL);
